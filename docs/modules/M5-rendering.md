...
| M5-FR-03 | 按输出语言出中/英/双语；纸张按语言/地区（中文 A4，英文缺省 A4）。 | MVP |
| M5-FR-04 | 照片开关（默认关，按地区给默认）；纯黑白开关。 | MVP |
| M5-FR-05 | 设计令牌参数化（§6），换风格=改令牌/换布局。 | MVP |
| M5-FR-06 | 降级（GR-6）：LLM 不可用时用规则把结构化数据直灌模板，仍出真实 ATS PDF。 | MVP |
| M5-FR-07 | 多套模板（技术/设计/学术/商务）。 | 后续 |
| M5-FR-08 | 导出 DOCX。 | 后续 |
| M5-FR-09 | 求职信（Cover Letter）。 | 后续 |
...
## 5. ATS 硬规则（渲染层强制，GR-4）
单栏；真文本 PDF（不得把文字渲染成图片）；嵌入标准字体；标准区块标题；关键信息放正文区不放页眉页脚；不用表格/文本框承载关键内容；日期统一 `YYYY.MM – YYYY.MM`；文件名 `姓名_岗位_简历.pdf`、填好 PDF 元数据。
