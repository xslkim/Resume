...
interview_sessions(
  ...
  created_at, updated_at)                      -- 见 M2 §6.7

jd(id, user_id, raw_text, lang, keywords_json, created_at)

generations(
  id, user_id, jd_id, params_json,             -- {length, language, template}
  status,                                       -- draft | final
  structured_data_json,                         -- JSON Resume + _source/_lang/_gaps
  created_at)

output_files(
  id, generation_id, type,                      -- pdf | docx
  path, meta_json, created_at)                   -- path 指向本机文件

audit_log(
  id, user_id, operation,                        -- interview|ingest|query|lint|render
  model, cost, source_trace, status, created_at) -- 见 M7-FR-06
```
...
| M8-FR-06 | （可选）配置私有 GitHub remote 并 `git push` 备份（SSH key/PAT 走环境变量）。 | 可选 |
| M8-FR-07 | 版本历史表 / 画像表 / 页索引缓存表。 | 后续 |
